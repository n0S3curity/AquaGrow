    ...
    function createSensorCard(sensor) {
        const statusColor = sensor.status.includes("DRY") ? 'bg-red-100 border-red-400 text-red-700' :
                            sensor.status.includes("Optimal") ? 'bg-green-100 border-green-400 text-green-700' :
                            'bg-blue-100 border-blue-400 text-blue-700';

        const buttonColor = sensor.status.includes("DRY") ? 'bg-red-500 hover:bg-red-600' : 'bg-blue-500 hover:bg-blue-600';
        ...
        return `
            <div class="bg-white p-6 rounded-xl shadow-lg border ${statusColor} flex flex-col space-y-3">
                ...
                <div class="flex justify-between items-center text-lg">
                    ...
                    <span class="text-3xl font-extrabold ${sensor.status.includes("DRY") ? 'text-red-600' : 'text-green-600'}">
    ...
    function updateMoistureChart(selectedSensorName) {
        ...
        if (selectedSensorName === 'all') {
            ...
            allSensorData.forEach((sensor, index) => {
                sensor.moisture_history.forEach(entry => allLabels.add(entry.timestamp));
                datasets.push({
                    label: sensor.name,
                    data: sensor.moisture_history.map(entry => entry.value),
                    borderColor: colors[index % colors.length],
                    tension: 0.1,
                    fill: false,
                    pointRadius: 3,
                    pointBackgroundColor: colors[index % colors.length],
                    originalTimestamps: sensor.moisture_history.map(entry => entry.timestamp)
                });
                datasets.push({
                    label: `${sensor.name} Threshold`,
                    data: new Array(sensor.moisture_history.length).fill(sensor.moisture_threshold),
            ...
            if (sensor) {
                dataToShow = sensor.moisture_history;
            ...
            const data = dataToShow.map(entry => entry.value);
